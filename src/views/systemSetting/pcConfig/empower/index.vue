...
import {
  bottomQrcodeGetApi,
  bottomQrcodeSaveApi,
  pcBottomAuthorizeSaveApi,
  pcBottomAuthorizGetApi,
...
export default {
  ...
  mounted() {
    ...
    if (checkPermi(['platform:pc:shopping:bottom:authorize:get'])) this.getPcAuthorize();
  ...
  methods: {
    ...
    //底部授权数据
    getPcAuthorize() {
      pcBottomAuthorizGetApi().then((res) => {
        Object.assign(this.formValidate, res);
      });
    },
    //底部授权保存
    handleAuthorizeSave(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.loadingBtn = true;
          pcBottomAuthorizeSaveApi(this.formValidate)
            .then((res) => {
              this.$message.success('保存成功');
              this.loadingBtn = false;
              this.getPcAuthorize();
        ...
        <div class="authorize">
          ...
          >
            <el-form-item label="联系电话:">
              <el-input v-model="formValidate.phone" size="small" class="from-ipt-width" placeholder="请输入联系电话" />
            </el-form-item>
            <el-form-item label="地址:">
              <el-input v-model="formValidate.address" size="small" class="from-ipt-width" placeholder="请输入地址" />
            </el-form-item>
            <el-form-item label="授权信息:" prop="authInfo">
              <el-input
                v-model="formValidate.authInfo"
                size="small"
                class="from-ipt-width"
                placeholder="请输入授权信息"
              />
            </el-form-item>
            <el-form-item label="备案号:" prop="filingNum">
              <el-input
                v-model="formValidate.filingNum"
                size="small"
                class="from-ipt-width"
                placeholder="请输入备案号"
              />
            </el-form-item>
            <el-form-item v-hasPermi="['platform:pc:shopping:bottom:authorize:save']">
              <el-button
                type="primary"
                v-debounceClick="
                  () => {
