<template>
  <div class="divBox relative">
    ...
    <!-- 批量增加销量-->
    <el-dialog title="增加其他销量" :visible.sync="dialogVisible" width="540px" :before-close="handleClose">
      <el-form :model="formData" ref="formData" label-width="100px" class="demo-ruleForm">
        <el-form-item label="增加其他销量：" required>
...
export default {
  ...
  methods: {
    ...
    //搜索
    handleSeachList() {
      console.log('handleSeachList 调用，当前 categoryId:', this.tableFrom.categoryId);
      this.getList(1);
      this.goodHeade();
    },
    //批量审核
    ...
    handlePreview(id) {
      ...
      if (this.ishttp(this.frontDomain)) {
        this.frontDomainUrl = `${this.frontDomain}/pages/goods/goods_details/index?id=${id}&mt=0`;
      } else {
        this.frontDomainUrl = `https://${this.frontDomain}/pages/goods/goods_details/index?id=${id}&mt=0`;
      }
    },
    getMerId(id) {
    ...
    goodHeade() {
      ...
      // 检查 categoryId 是否为三级分类
      if (data.categoryId) {
        const selectedCategory = this.findCategoryById(data.categoryId, this.adminProductClassify);
        if (selectedCategory && selectedCategory.childList && selectedCategory.childList.length > 0) {
          // 如果不是三级分类，不传 categoryId
          data.categoryId = null;
        }
      }
      
      // 注意：不要删除 categoryId，因为统计头部数量时也需要按分类过滤
      console.log('goodHeade 发送参数:', JSON.stringify(data, null, 2));
    ...
    getList(num) {
      ...
      // 在发送请求前检查 categoryId 是否为三级分类
      let sendParams = { ...this.tableFrom };
      if (sendParams.categoryId) {
        const selectedCategory = this.findCategoryById(sendParams.categoryId, this.adminProductClassify);
        if (selectedCategory && selectedCategory.childList && selectedCategory.childList.length > 0) {
          // 如果不是三级分类，不传 categoryId
          sendParams.categoryId = null;
          this.$message.warning('检测到选择的不是三级分类，已自动清除分类条件');
        }
      }
      
      console.log('getList 发送参数:', JSON.stringify(sendParams, null, 2));
      this.listLoading = true;
      productLstApi(sendParams)
        .then((res) => {
    ...
    handleCategoryChange(value) {
      console.log('handleCategoryChange 调用，当前 categoryId:', value);
      
      if (value) {
        // 查找选中的分类
        const selectedCategory = this.findCategoryById(value, this.adminProductClassify);
        console.log('选中的分类:', selectedCategory);
        
        // 检查是否有子分类，如果有子分类则不是三级分类
        if (selectedCategory && selectedCategory.childList && selectedCategory.childList.length > 0) {
          this.$message.warning('请选择三级分类，不能选择有子分类的分类');
          // 清空选择
          this.tableFrom.categoryId = null;
          this.$nextTick(() => {
            if (this.$refs.cascader) {
      ...
      this.tableFrom.categoryId = value;
    ...
    findCategoryById(id, categories) {
      ...
      for (let category of categories) {
        ...
        if (category.childList && category.childList.length > 0) {
          const found = this.findCategoryById(id, category.childList);
