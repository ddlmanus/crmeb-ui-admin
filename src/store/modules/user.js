...
const state = {
  ...
  menuList: (() => {
    try {
      const menuData = localStorage.getItem('MerPlatAdmin_MenuList');
      return menuData && menuData !== 'undefined' ? JSON.parse(menuData) : [];
    } catch (e) {
      console.warn('菜单数据解析失败:', e);
  ...
  oneLvRoutes: (() => {
    try {
      const routeData = localStorage.getItem('MerPlatAdmin_oneLvRoutes');
      return routeData && routeData !== 'undefined' ? JSON.parse(routeData) : [];
    } catch (e) {
      console.warn('路由数据解析失败:', e);
